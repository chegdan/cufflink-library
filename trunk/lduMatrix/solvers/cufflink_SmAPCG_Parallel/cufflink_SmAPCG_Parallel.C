...
#include "cufflink_SmAPCG_Parallel.H"
#include "cuda.h"
...
extern "C" void CFL_SmAPCG_Parallel(cusp_equation_system *,  OFSolverPerformance *, const cpuInterfaces *);//extern function in CUDA that solves the equation system

// * * * * * * * * * * * * * * Static Data Members * * * * * * * * * * * * * //

namespace Foam
{
    defineTypeNameAndDebug(cufflink_SmAPCG_Parallel, 0);

    lduSolver::addsymMatrixConstructorToTable<cufflink_SmAPCG_Parallel>
        addcufflink_SmAPCG_ParallelSymMatrixConstructorToTable_;
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::cufflink_SmAPCG_Parallel::cufflink_SmAPCG_Parallel
(
...
Foam::lduSolverPerformance Foam::cufflink_SmAPCG_Parallel::solve
(
    scalarField& x,
    const scalarField& b,
...
) const
{
    ...
    lduSolverPerformance solverPerf("cufflink_SmAPCG_Parallel", fieldName());
	...
	CFL_SmAPCG_Parallel(&CES, &OFSP, &OFInterfaces);//call the CUDA code to solve the system
