...
extern "C" void CFL_DiagPBiCGStab(cusp_equation_system *CES,  OFSolverPerformance *OFSP ){
...
//	A.sort_by_row();//coo must be sorted by row.  Improves perfomance by 2X normal with this line
	...
	#include "../CFL_Headers/setGPUStorage.H"
		...
		if(OFSP->debugCusp){
			printf("   Using Cusp_Diagonal preconditioner\n");}
...
//end Krylov Solver
	...
	if(OFSP->debugCusp){std::cout << "   Iteration "<<count<<" residual = "<< std::setw(10) << normR << std::endl;}//do we need this here?*/
