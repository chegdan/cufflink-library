...
This file is part of cufflink.
    ...
    Description
        Sparse approximate inverse preconditioned conjugate gradient 
	solver for symmetric Matrices using a CUSP CUDA™ based solver.
                                                            
\**********************************************************************/

extern "C" void CFL_AinvPCG(cusp_equation_system *CES,  OFSolverPerformance *OFSP )
{
	...
	if(OFSP->debugCusp){
		std::cout<<"   Using Cusp_Ainv preconditioner - Bridson Ainv with novel dropping strategy and linStrategy = "<< OFSP->linStrategy<<"\n";
	}
