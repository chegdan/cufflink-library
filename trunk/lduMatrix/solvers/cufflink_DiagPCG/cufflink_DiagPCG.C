...
extern "C" void CFL_DiagPCG(cusp_equation_system *CES,  OFSolverPerformance *OFSP);//extern function in CUDA that solves the equation system

// * * * * * * * * * * * * * * Static Data Members * * * * * * * * * * * * * //

namespace Foam
{
    defineTypeNameAndDebug(cufflink_DiagPCG, 0);

    lduSolver::addsymMatrixConstructorToTable<cufflink_DiagPCG>
        addcufflink_DiagPCGSymMatrixConstructorToTable_;
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::cufflink_DiagPCG::cufflink_DiagPCG
(
...
Foam::lduSolverPerformance Foam::cufflink_DiagPCG::solve
(
    scalarField& x,
    const scalarField& b,
...
//grab the preconditioner type up here readInt(dict().lookup("storage")) or readLabel(dict().lookup("storage"))

/*if(lduMatrix::preconditioner::getName(dict()) != "cusp"){
        FatalErrorIn("set preconditioner to cusp in fvSolution file")
            << "Fatal Error"
            << abort(FatalError);
}*/
...
//#include "../CFL_Headers/getPreconditioner.H"
#include "../CFL_Headers/getGPUStorage.H"

 // --- Setup class containing solver performance data
    lduSolverPerformance solverPerf("cufflink_DiagPCG",fieldName());
	...
	CFL_DiagPCG(&CES, &OFSP);//call the CUDA code to solve the system
